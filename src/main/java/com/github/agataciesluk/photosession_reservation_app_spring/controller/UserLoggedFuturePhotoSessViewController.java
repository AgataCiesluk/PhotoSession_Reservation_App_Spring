...
public class UserLoggedFuturePhotoSessViewController {
    ...
    @GetMapping
    public String showFuturePhotoSess(Model model, @AuthenticationPrincipal CurrentUser currentUser) {
    ...
    public List<PhotoSessionEntity> futureSessions(@AuthenticationPrincipal CurrentUser currentUser) {
        if ("admin".equals(currentUser.getUsername())) {
            return photoSessionBasicService.getAllFutureSessSorted();
        }
        return photoSessionBasicService.getFutureSessByUsernameSorted(currentUser.getUsername());
    }
}
